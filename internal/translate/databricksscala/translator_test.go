...
func TestTranslate_SimpleObject(t *testing.T) {
	...
	assert.Contains(t, result, "val users_schema = StructType(Seq(")
...
func TestTranslate_NestedObject(t *testing.T) {
	...
	assert.Contains(t, result, "val _Address = StructType(Seq(")
...
func TestTranslate_TopologicalOrder(t *testing.T) {
	...
	addressIdx := strings.Index(result, "val _Address = StructType")
	customerIdx := strings.Index(result, "val _Customer = StructType")
...
func TestTranslate_ChainedDeps(t *testing.T) {
	...
	aIdx := strings.Index(result, "val _A = StructType")
	bIdx := strings.Index(result, "val _B = StructType")
	cIdx := strings.Index(result, "val _C = StructType")
...
func TestTranslate_NoDefs(t *testing.T) {
	...
	assert.Contains(t, result, "val simple_schema = StructType(Seq(")
	lines := strings.Split(result, "\n")
	schemaCount := 0
	for _, line := range lines {
		if strings.Contains(line, "val ") && strings.Contains(line, " = StructType") {
			schemaCount++
		}
	}
...
func TestTranslate_ArrayOfObjects(t *testing.T) {
	...
	assert.Contains(t, result, "val _Items = StructType(Seq(")
	assert.Contains(t, result, "ArrayType(_Items)")
}

func TestTranslate_WithComment(t *testing.T) {
	schema := &jsonschema.Schema{
		Type:     "object",
		Required: []string{"id"},
		Properties: map[string]*jsonschema.Schema{
