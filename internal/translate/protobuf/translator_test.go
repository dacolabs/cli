...
func TestTranslate_FieldNameSanitization(t *testing.T) {
	schema := &jsonschema.Schema{
		Type: "object",
		Properties: map[string]*jsonschema.Schema{
			"foo-bar":    {Type: "string"},
			"1st_place":  {Type: "integer"},
	...
	assert.Contains(t, result, "string foo_bar")
	assert.Contains(t, result, "int64 _1st_place")
	assert.Contains(t, result, "bool camelcase")
	assert.Contains(t, result, "string with_space")
}
